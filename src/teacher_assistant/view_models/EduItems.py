...
class EduItemViewModel(NotifyPropertyChanged):
    ...
    def update_value(self,column_name, value):
        msg =''
        try:
            self.db_cursor.execute(f"UPDATE educational_resources SET {column_name} = '{value}' WHERE Id = {self.Id};")
...
class EduItemStudentViewModel(NotifyPropertyChanged):
    ...
    def __init__(self, model:EduItemStudentService):
...
class ClassroomGroupViewModel():
    ...
    def __init__(self):#, model:EduItems.ClassroomGroupService=None):
        super().__init__()
        ...
        self.model = ClassroomGroupService()
