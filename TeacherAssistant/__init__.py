...
from TeacherAssistant.utils.Json import JSONManager
...
# Use a class or dictionary for mutable state
class GlobalState:
    ...
    def setup_app_directories(self):
        ...
        settings_path = self.appdata_path +'\\settings.json'
        settings_manager.set_path(settings_path)

        # Set the config manager path
        config_path = self.application_path +'\\resources\\templates\\01-Quiz-config.json'
        template_config.set_path(config_path)
...
settings_manager = JSONManager()
template_config = JSONManager()

theme_manager = utils.ThemeManager()
