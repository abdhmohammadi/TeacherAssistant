...
from PySide6.QtWidgets import (QHBoxLayout, QVBoxLayout, QMenu, QWidgetAction, QProgressBar, QSizePolicy,
...
class ObservedBehaviourWidget(QWidget):
    ...
    def submit_form(self):
        ...
        insert_query = 'INSERT INTO observed_behaviours(date_time_,student_id, observed_behaviour_, analysis_) VALUES (%s, %s, %s,%s)'

        app_context.database.execute(insert_query, (now,student_id,obeh,ta))
...
class EduItemStudentWidget(QWidget):
    ...
    def __init__(self, id:int =0, main_content='', answer='', feedback ='', 
                 ...
                 sent_date:datetime=None, deadline:datetime=None, 
                 received_date ='',status =''):
        
        super().__init__()
        
        self.viewModel = EduItemStudentViewModel(EduItemStudentService())
...
class EduItemWidget(QWidget):
    ...
    def __init__(self, data= None, width= 400, parent=None, cursor=None):
    ...
    def select_item(self,sender:QPushButton):
        ...
        sender.setIcon(QIcon(':icons/check.svg') if self._view_model.selected else QIcon())
