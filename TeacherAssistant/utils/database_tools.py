...
def create_database(connection:psycopg2.extensions.connection, database: str):
    """
...
def initialize_database(connection:psycopg2.extensions.connection):
    """
...
def change_database_in_session(connection:psycopg2.extensions.connection, database: str,password) -> bool:
    """
    ...
    try:
        ...
        connection = psycopg2.connect(host=params['host'], port=params['port'], 
...
def get_postgres_columns(table_name, cursor):
...
def bulk_insert_csv(csv_file, table_name, column_mapping, cursor, chunk_size = 10000):
    """Optimized bulk insert with better memory usage and error handling."""
    try:
        # Load CSV in chunks for large files
        chunksize = chunk_size
        total_rows = 0
        print('columns:', column_mapping.keys())
        for chunk in pd.read_csv(csv_file, chunksize=chunk_size, dtype_backend='numpy_nullable'):
...
def restore_postgres_db(backup_dump_file, target_db, user, password, host, port, overwrite = False, pg_bin_path =r"C:\\Program Files\\PostgreSQL\\17\\bin"):
    ...
    PG_PSQL_PATH = pg_bin_path + r"\\psql.exe"
    PG_CREATEDB_PATH = pg_bin_path + r"\\createdb.exe"
    PG_RESTORE_PATH = pg_bin_path + r"\\pg_restore.exe"

    env = os.environ.copy()
    env['PGPASSWORD'] = password

    def run_command(cmd, error_prefix):
        try:
            ...
            if process.returncode != 0:
                return False, f"{error_prefix}: {''.join(output)}"
            return True, ""
        
        except Exception as e:
            return False, f"{error_prefix}: {str(e)}"
    ...
    if not overwrite:
        try:
            ...
            if not db_exists:
                ...
                success, msg = run_command(create_cmd, "Database creation failed")
            
                if not success: return False, msg, None
            else:
                return False, 'Overwrite not allowed by user.', target_db
            
        except Exception as e:
            return False, f"❌ Database check failed: {str(e)}", None

    # 2. Perform Force Restore
    if overwrite:
        print(f"💥 Force-restoring to '{target_db}'...")
        restore_cmd = [
            PG_RESTORE_PATH,
            '-h', host,
            '-p', str(port),
        ...
        success, msg = run_command(restore_cmd, "Restore failed")

        if not success: return False, msg, target_db

        return True, f"✅ Successfully restored '{target_db}'", target_db
