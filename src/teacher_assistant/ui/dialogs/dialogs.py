...
from PySide6.QtWidgets import (QMessageBox, QVBoxLayout,QTableWidget,QFormLayout, QLineEdit, QPushButton, 
                               QListView, QHBoxLayout,QGridLayout,QCalendarWidget, QComboBox, QDateEdit,
...
class CustomAssignmentDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        ...
        # Form layout for organized labeling
        form_layout = QFormLayout()

        # Multi-line text input (description)
        self.description_edit = QTextEdit()
        self.description_edit.setPlaceholderText("Enter the assignment description here...")
        form_layout.addRow("Description:", self.description_edit)

        self.feedback_edit = QTextEdit()
        self.feedback_edit.setPlaceholderText("Enter the feedback description here...")
        form_layout.addRow("Feedback:", self.feedback_edit)

        # Maximum score
        self.max_score_edit = QLineEdit()
        self.max_score_edit.setPlaceholderText("e.g., 100")
        form_layout.addRow("Maximum Score:", self.max_score_edit)

        # Score earned
        self.score_earned_edit = QLineEdit()
        self.score_earned_edit.setPlaceholderText("e.g., 85")
        form_layout.addRow("Score Earned:", self.score_earned_edit)
        ...
        form_layout.addRow("Assignment Date:", self.assignment_date_edit)

        # Response/Submission date
        self.response_date_edit = QDateEdit()
        self.response_date_edit.setCalendarPopup(True)
        self.response_date_edit.setDate(QDate.currentDate())
        form_layout.addRow("Response Date:", self.response_date_edit)
        ...
        self.save_button.clicked.connect(self.accept)    # Closes with Accepted
        self.cancel_button.clicked.connect(self.reject)  # Closes with Rejected
    ...
    def get_data(self):
        """Convenient method to retrieve all entered values as a dictionary."""
        return {
            "description": self.description_edit.toHtml(),
            "feedback": self.feedback_edit.toHtml(),
            "max_score": self.max_score_edit.text().strip(),
            "score_earned": self.score_earned_edit.text().strip(),
            "deadline": self.deadline_edit.date().toString("yyyy-MM-dd"),
            "assignment_date": self.assignment_date_edit.date().toString("yyyy-MM-dd"),
            "response_date": self.response_date_edit.date().toString("yyyy-MM-dd"),
        }
